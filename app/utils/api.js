 ...
 * @param {string} period - Number of days (default: 30)
 */
export async function getSalesAnalytics(period = '30') {
  try {
    const res = await api.get(`/dashboard/analytics?period=${period}`);
    return res.data;
  } catch (err) {
    throw new Error(
      err.response?.data?.message || err.message || "Failed to get sales analytics",
    );
  }
}
...
export async function getInventoryAnalytics() {
  try {
    ...
    throw new Error(
      err.response?.data?.message || err.message || "Failed to get inventory analytics",
    );
  }
}
 ...
 * @param {string} period - Number of days (default: 30)
 */
export async function getCustomerAnalytics(period = '30') {
  try {
    const res = await api.get(`/dashboard/customers?period=${period}`);
    return res.data;
  } catch (err) {
    throw new Error(
      err.response?.data?.message || err.message || "Failed to get customer analytics",
    );
  }
}
